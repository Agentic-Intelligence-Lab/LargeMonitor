...
def evaluate_till_now(model: torch.nn.Module, data_loader, device, task_id=-1, acc_matrix=None, args=None):
    ...
    eval_summary = {
        'sub_task': task_id,
        'avg_Acc@1': float(avg_stat[0]),
        'avg_Acc@5': float(avg_stat[1]),
        'avg_Loss': float(avg_stat[2]),
        'current_Acc@1': float(test_stats['Acc@1']),
        'current_Acc@5': float(test_stats['Acc@5']),
        'current_Loss': float(test_stats['Loss']),
    }
    for i in range(task_id + 1):
        eval_summary[f'task{i}_Acc@1'] = float(stat_matrix[0, i])
        eval_summary[f'task{i}_Acc@5'] = float(stat_matrix[1, i])
        eval_summary[f'task{i}_Loss'] = float(stat_matrix[2, i])
    ...
    if task_id > 0:
        ...
        forgetting = 0.0
        backward = 0.0
    eval_summary['forgetting'] = float(forgetting)
    eval_summary['backward'] = float(backward)
    if task_id > 0:
...
def _add_new_lora(model, device, ckpt_path):
...
def train_and_evaluate(model: torch.nn.Module, criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer, device: torch.device, args=None, class_mask=None):
    ...
    omega_As=[] 
    omega_Bs=[]
    ckpt_path = utils.init_ckpt_path(args.output_dir or './output', loglevel=logging.ERROR)
    decision_log = os.path.join(args.output_dir, 'decision.jsonl') if args.output_dir else None
    eval_log = os.path.join(args.output_dir, 'eval_stats.jsonl') if args.output_dir else None

    for task_id in tqdm(range(args.num_tasks), desc='Tasks'):
        if args.lora_per_task and task_id > 0 and args.new_lora:
            model = _add_new_lora(model, device, ckpt_path)
            loss_window.clear()
            new_peak_detected = True
            if decision_log:
            ...
            tqdm.write(f'Task {task_id + 1}/{args.num_tasks}: new LoRA added (lora_per_task)')

        for inputs, labels in data_loader[task_id]['train']:
                ...
                if len(batch) == args.nb_batch:
                    ...
                    if loss_window_mean < args.loss_window_mean_threshold and loss_window_variance < args.loss_window_variance_threshold and new_peak_detected:
                        ...
                        if args.new_lora and not args.lora_per_task:
                            model = _add_new_lora(model, device, ckpt_path)
                            new_lora_added = True

                    if decision_log:
                        with open(decision_log, 'a') as f:
                            f.write(json.dumps({'sub_task': task_id, 'new_lora_added': new_lora_added}) + '\n')
        ...
        eval_summary = evaluate_till_now(model=model, data_loader=data_loader, device=device, 
                                    task_id=task_id, acc_matrix=acc_matrix, args=args)
        tqdm.write(
            f'Task {task_id + 1}/{args.num_tasks} done | '
            f'Avg Acc@1 {eval_summary["avg_Acc@1"]:.2f} | '
            f'Current Acc@1 {eval_summary["current_Acc@1"]:.2f}'
        )
