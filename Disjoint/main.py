...
def main(args):
    ...
    vit_model = create_model(args.model, pretrained=False, checkpoint_path=args.checkpoint_path)
...
if __name__ == '__main__':
    ...
    elif config == 'imagenet_cu_online_lora':
        from configs.imagenet_cu_online_lora import get_args_parser
        config_parser = subparser.add_parser(
            'imagenet_cu_online_lora',
            help='ImageNet-CU stream (largemonitor, 10 tasks)',
        )
    ...
    config_parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Add new LoRA at the start of each new task (skip loss-window detection)',
    )

    if config == 'imagenet_cu_online_lora':
