...
class Imagenet_Sketch(ImageFolder):
    def __init__(
        ...
        self.path = self.root + "/sketch/"
        super().__init__(self.path, transform=transform, target_transform=target_transform)
        ...
        if train:
            ...
            samples = [self.samples[idx] for idx in self.train_sample]
            self.targets = [s[1] for s in samples]
            self.samples = samples
        else:
            self.classes = [i for i in range(1000)]
            self.class_to_idx = [i for i in range(1000)]
            samples = [self.samples[idx] for idx in self.test_sample]
            self.targets = [s[1] for s in samples]
            self.samples = samples
